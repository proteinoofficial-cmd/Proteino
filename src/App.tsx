...
export default function App() {
  ...
  const [cartGymSearch, setCartGymSearch] = useState('');
  ...
  useEffect(() => {
    ...
    const handleSyncEvent = (e?: any) => {
      if (e?.detail?.orderId && e?.detail?.status) {
        setOrders(prev => prev.map(o => o.id === e.detail.orderId ? {
          ...
          acceptedAt: (e.detail.status === 'cooking' || e.detail.status === 'accepted' || e.detail.status === 'out_for_delivery') ? (o.acceptedAt || e.detail.acceptedAt || new Date().toISOString()) : o.acceptedAt,
          deliveredAt: e.detail.status === 'delivered' ? (o.deliveredAt || e.detail.deliveredAt || new Date().toISOString()) : o.deliveredAt,
          declinedAt: e.detail.status === 'declined' ? (o.declinedAt || e.detail.declinedAt || new Date().toISOString()) : o.declinedAt,
          declinedDate: e.detail.status === 'declined' ? (o.declinedDate || e.detail.declinedDate) : o.declinedDate,
        ...
        bc.onmessage = (event) => {
          if (event.data?.orderId && event.data?.status) {
            setOrders(prev => prev.map(o => o.id === event.data.orderId ? {
              ...
              acceptedAt: (event.data.status === 'cooking' || event.data.status === 'accepted' || event.data.status === 'out_for_delivery') ? (o.acceptedAt || event.data.acceptedAt || new Date().toISOString()) : o.acceptedAt,
              deliveredAt: event.data.status === 'delivered' ? (o.deliveredAt || event.data.deliveredAt || new Date().toISOString()) : o.deliveredAt,
              declinedAt: event.data.status === 'declined' ? (o.declinedAt || event.data.declinedAt || new Date().toISOString()) : o.declinedAt,
              declinedDate: event.data.status === 'declined' ? (o.declinedDate || event.data.declinedDate) : o.declinedDate,
    ...
    window.addEventListener('proteino_subscription_declined', handleSubDeclinedDirect);
    ...
    return () => {
      ...
      window.removeEventListener('proteino_subscription_declined', handleSubDeclinedDirect);
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col justify-between select-none">
      ...
      <div className="w-full max-w-md mx-auto bg-[#FAF9F6] min-h-screen shadow-2xl flex flex-col relative overflow-x-hidden border-x border-slate-200/50">
                  ...
                  {cart.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-4 bg-white border border-slate-200/50 rounded-3xl p-4 shadow-xs">
                        ...
                        {/* Partner Gym Selector (Same system as Monthly Plans: Search + 3 visible gyms + scroll) */}
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center justify-between">
                            <label className="text-[9px] font-black uppercase tracking-wider text-brand-navy/40 flex items-center gap-1">
                              <MapPin className="w-3.5 h-3.5 text-brand-green" />
                              <span>Select Partner Gym Drop-off Desk</span>
                            </label>
                          ...
                          <div className="relative">
                            <input
                              ...
                              value={cartGymSearch}
                              onChange={(e) => setCartGymSearch(e.target.value)}
                          ...
                          <div className="grid grid-cols-1 gap-2 max-h-[224px] overflow-y-auto pr-1 scroll-smooth">
                            {(() => {
                              const filteredGyms = GYMS.filter(gym => 
                                gym.name.toLowerCase().includes(cartGymSearch.toLowerCase()) || 
                                gym.location.toLowerCase().includes(cartGymSearch.toLowerCase())
                              );

                              if (filteredGyms.length === 0) {
                                return (
                                  <div className="text-center py-5 px-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                    <p className="text-xs font-bold text-brand-navy/60">No gyms matched "{cartGymSearch}"</p>
                                    <button
                                      type="button"
                                      onClick={() => setCartGymSearch('')}
                                      className="mt-1.5 text-[11px] font-black text-brand-green underline cursor-pointer"
                                    >
                                ...
                                >
                                  ...
                                  <div className="flex-grow min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      <h4 className="font-black text-xs text-brand-navy leading-tight truncate">{gym.name}</h4>
                                      {selectedGymId === gym.id && (
        ...
        {/* Customer Notification Floating Popup: Subscription Declined by Admin */}
        <AnimatePresence>
          {customerDeclinedSubAlert && (
            <motion.div
              initial={{ opacity: 0, y: -40, scale: 0.95 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: -20, scale: 0.95 }}
              transition={{ type: 'spring', damping: 22, stiffness: 280 }}
              className="fixed top-5 inset-x-4 max-w-sm mx-auto z-[135] pointer-events-auto"
            >
              <div className="bg-[#1A0B0E] text-white p-4 rounded-2xl shadow-2xl border-2 border-red-500/80 flex items-start justify-between gap-3 backdrop-blur-md">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-red-500/20 text-red-400 flex items-center justify-center shrink-0 border border-red-500/30 mt-0.5">
                    <AlertCircle className="w-5 h-5 text-red-400 stroke-[2.5]" />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs sm:text-sm font-black text-white">Subscription Declined</span>
                      <span className="text-[8.5px] font-black uppercase tracking-wider text-red-300 bg-red-900/60 px-2 py-0.5 rounded-full border border-red-700/50 shrink-0">
                    ...
                    <div className="mt-2.5 flex items-center gap-2">
                      <button
                        ...
                        className="text-[10px] font-black bg-red-600 hover:bg-red-500 text-white px-3 py-1.5 rounded-xl transition-colors cursor-pointer shadow-xs"
                      >
                        Explore Plans
                      </button>
                    </div>
                ...
                <button
                  type="button"
                  onClick={() => setCustomerDeclinedSubAlert(null)}
                  className="p-1 rounded-lg text-white/40 hover:text-white transition-colors cursor-pointer shrink-0"
